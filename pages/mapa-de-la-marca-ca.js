...
const MapByMarcaCa = (props) => (
  <Layout>
    ...
    <p className='align-center'><small><Link prefetch as={`/m-o-g-m-ca/${props.markers[0].marca.term_id}/${props.markers[0].marca.slug}`} href={`/ofertas-de-la-marca-ca?id=${props.markers[0].marca.term_id}`}><a>ver listado</a></Link></small></p>
       ...
       <GoogleMapReact
        ...
        {props.markers.map((marker, index) => (
          ...
          <MarkerComponent
            ...
            text={<Link prefetch as={`/ogmca/${marker.ID}/${marker.slug}`} href={`/oferta-gran-marca-ca?id=${marker.ID}`}><a title={marker.name}><span><img src={'/static/32/' + props.markers[0].marca.slug +'-familias-numerosas.png'} /></span></a></Link>}
          />
              ))}
        </GoogleMapReact>
...
MapByMarcaCa.getInitialProps = async function(context) {
  ...
  const res = await fetch(`https://gestorbeneficios.familiasnumerosas.org/wp-json/lanauva/v1/of_gr_m_ca?marca=${id}&sim-model=name-id-slug-lat-lon-marca`)
